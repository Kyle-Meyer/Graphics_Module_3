      ...
      vao_(0), vertex_buffer_(0)
{
    ...
    std::cout << "DraggableLineGeometryNode: Created with start point (" 
              << start_point_.x << ", " << start_point_.y << ")" << std::endl;
}
...
{
    std::cout << "DraggableLineGeometryNode: Creating OpenGL resources..." << std::endl;
    ...
    std::cout << "Generated VAO: " << vao_ << ", VBO: " << vertex_buffer_ << std::endl;
    ...
    std::cout << "DraggableLineGeometryNode: Created successfully!" << std::endl;
    return true;
}
...
void DraggableLineGeometryNode::setup_vertex_data()
{
    ...
    std::cout << "  Start: (" << vertices[0].x << ", " << vertices[0].y 
              << ") Color: (" << vertices[0].r << ", " << vertices[0].g << ", " << vertices[0].b << ", " << vertices[0].a << ")" << std::endl;
    std::cout << "  End: (" << vertices[1].x << ", " << vertices[1].y 
              << ") Color: (" << vertices[1].r << ", " << vertices[1].g << ", " << vertices[1].b << ", " << vertices[1].a << ")" << std::endl;
    ...
    std::cout << "Uploaded " << sizeof(vertices) << " bytes to VBO" << std::endl;
}
...
void DraggableLineGeometryNode::setup_vertex_attributes()
{
    ...
    std::cout << "Vertex attributes configured:" << std::endl;
    std::cout << "  Position (loc 0): 2 floats, stride " << VERTEX_STRIDE << ", offset " << POSITION_OFFSET << std::endl;
    std::cout << "  Color (loc 1): 4 floats, stride " << VERTEX_STRIDE << ", offset " << COLOR_OFFSET << std::endl;
}
...
void DraggableLineGeometryNode::draw(SceneState& scene_state)
{
    if(vao_ == 0)
    {
        std::cout << "Warning: DraggableLineGeometryNode not initialized, call create() first" << std::endl;
        return;
    }
...
void DraggableLineGeometryNode::destroy()
{
    ...
    std::cout << "DraggableLineGeometryNode: OpenGL resources cleaned up" << std::endl;
}
